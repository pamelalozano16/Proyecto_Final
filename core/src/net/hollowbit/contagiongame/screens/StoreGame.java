...
public class StoreGame implements Screen {
	...
	public StoreGame(final ContagionGame game) {
		...
		Timer.schedule(new Task(){
            ...
            public void run() {
    			...
    			fruit_x =(int) (Math.random() * ContagionGame.WIDTH-200)+100;
		...
		Timer.schedule(new Task(){
            ...
            public void run() {
    			//Posicion random en x
    			virus_x =(int) (Math.random() * ContagionGame.WIDTH-200)+100;	
	...
	public void render(float delta) {
		...
		if(!start) { //Todavia no empieza el juego entonces pinta el button Play
			boolean playbtn = game.buttonCollision(playButtonActive, playButtonInactive, PLAY_BTN_X, PLAY_BTN_Y, PLAY_BTN_WIDTH, PLAY_BTN_HEIGHT, 50, 40);
	...
	void movement(Sprite player) {
		...
		if(game.WIDTH-100<PLAYER_X) {PLAYER_X=game.WIDTH-100;}
