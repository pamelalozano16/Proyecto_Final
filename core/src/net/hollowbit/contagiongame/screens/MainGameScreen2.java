...
public class MainGameScreen2 implements Screen {
	...
	public MainGameScreen2(ContagionGame game) {
		this.game = game;
	...
	public void render(float delta) {
		...
		if (text==0) {
			...
			game.batch.draw(next,game.WIDTH-160,50, 60, 60);
		}
		if (text==1) {
			game.batch.draw(text2,50, 0, game.WIDTH-100, 300);
			game.batch.draw(next,game.WIDTH-160,50, 60, 60);
		}
		if (text==2) {
			game.batch.draw(text3,50, 0, game.WIDTH-100, 300);
			game.batch.draw(next,game.WIDTH-160,50, 60, 60);
		}
		if (text==3) {
			game.batch.draw(text4,50, 0, game.WIDTH-100, 300);
			game.batch.draw(next,game.WIDTH-160,50, 60, 60);
		}
		
		//Click de next
		imageCollision2(game.WIDTH-160, 50, 60, 60);
